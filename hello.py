...
def run_workflow():
    ...
    if (req_data['type'] == 'cwl'):
        ...
        subprocess.Popen('timeout '+req_data['timeout']+' bash cwl_run.sh ' + cwl_path+" "+ yaml_path+" "+ os.path.abspath(job_store_path)+" "+
        os.path.abspath(out_dir), shell=True)
    elif (req_data['type'] == 'toil'):
        toil_path = os.path.join(config.TOIL, 'main.py')

        subprocess.Popen(['timeout',req_data['timeout'],'python', config.TOIL +req_data["workflow"]+"/main.py", job_store_path,out_dir])
    else:
